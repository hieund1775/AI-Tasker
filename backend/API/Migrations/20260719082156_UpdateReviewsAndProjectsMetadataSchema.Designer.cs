// <auto-generated />
using System;
using AITasker_Modular.Database;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace AITasker_Modular.Migrations
{
    [DbContext(typeof(DataContext))]
    [Migration("20260719082156_UpdateReviewsAndProjectsMetadataSchema")]
    partial class UpdateReviewsAndProjectsMetadataSchema
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            MySqlModelBuilderExtensions.AutoIncrementColumns(modelBuilder);

            modelBuilder.Entity("AITasker_Modular.Database.ProposalAiChat", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AiResponse")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("ExpertId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("JobPostId")
                        .HasColumnType("char(36)");

                    b.Property<string>("UserMessage")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("ExpertId");

                    b.HasIndex("JobPostId");

                    b.ToTable("ProposalAiChats");
                });

            modelBuilder.Entity("AITasker_Modular.Database.SystemTransactionLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Description")
                        .HasColumnType("longtext");

                    b.Property<Guid>("ProjectId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("SystemTransactionLogs");
                });

            modelBuilder.Entity("AITasker_Modular.Database.SystemWallet", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<decimal>("TotalBalance")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime(6)");

                    b.HasKey("Id");

                    b.ToTable("SystemWallets");

                    b.HasData(
                        new
                        {
                            Id = new Guid("11111111-1111-1111-1111-111111111111"),
                            TotalBalance = 0m,
                            UpdatedAt = new DateTime(2026, 7, 19, 8, 21, 55, 521, DateTimeKind.Utc).AddTicks(8904)
                        });
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.Domain", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("Domains");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.DomainExpertProfile", b =>
                {
                    b.Property<Guid>("DomainId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("ExpertProfilesUserId")
                        .HasColumnType("char(36)");

                    b.HasKey("DomainId", "ExpertProfilesUserId");

                    b.HasIndex("ExpertProfilesUserId");

                    b.ToTable("DomainExpertProfiles");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.ExpertProfileSkill", b =>
                {
                    b.Property<Guid>("ExpertProfilesUserId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("SkillsId")
                        .HasColumnType("char(36)");

                    b.HasKey("ExpertProfilesUserId", "SkillsId");

                    b.HasIndex("SkillsId");

                    b.ToTable("ExpertProfileSkill");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.Skill", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("Skills");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.Specialization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("DomainId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("DomainId");

                    b.ToTable("Specializations");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ChatModule.Conversation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("ClientId")
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("ExpertId")
                        .HasColumnType("char(36)");

                    b.Property<Guid?>("OriginJobPostId")
                        .HasColumnType("char(36)");

                    b.HasKey("Id");

                    b.HasIndex("ClientId");

                    b.HasIndex("ExpertId");

                    b.HasIndex("OriginJobPostId");

                    b.ToTable("Conversations");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ChatModule.Message", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<Guid>("ConversationId")
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("IsRead")
                        .HasColumnType("tinyint(1)");

                    b.Property<Guid>("SenderId")
                        .HasColumnType("char(36)");

                    b.HasKey("Id");

                    b.HasIndex("ConversationId");

                    b.HasIndex("SenderId");

                    b.ToTable("Messages");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.DisputeModule.Dispute", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("ClientEvidenceUrl")
                        .HasColumnType("longtext");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("EvidenceDeadline")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("ExpertEvidenceUrl")
                        .HasColumnType("longtext");

                    b.Property<Guid?>("HandlerStaffId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("ProjectId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("ResolutionVerdict")
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("HandlerStaffId");

                    b.HasIndex("ProjectId");

                    b.ToTable("Disputes");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.DisputeModule.Report", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AdminNote")
                        .HasColumnType("longtext");

                    b.Property<string>("ClientExplanation")
                        .HasColumnType("longtext");

                    b.Property<string>("ClientExplanationDescription")
                        .HasColumnType("longtext");

                    b.Property<string>("ClientExplanationDesiredResolution")
                        .HasColumnType("longtext");

                    b.Property<string>("ClientExplanationEvidence")
                        .HasColumnType("longtext");

                    b.Property<string>("ClientExplanationReason")
                        .HasColumnType("longtext");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("CurrentRoundClientSubmitted")
                        .HasColumnType("tinyint(1)");

                    b.Property<bool>("CurrentRoundExpertSubmitted")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Description")
                        .HasColumnType("longtext");

                    b.Property<string>("DesiredResolution")
                        .HasColumnType("longtext");

                    b.Property<string>("DisputeType")
                        .HasColumnType("longtext");

                    b.Property<decimal>("EscrowPayExpert")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("EscrowRefundClient")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("EvidenceUrl")
                        .HasColumnType("longtext");

                    b.Property<string>("ExpertExplanation")
                        .HasColumnType("longtext");

                    b.Property<string>("ExpertExplanationDescription")
                        .HasColumnType("longtext");

                    b.Property<string>("ExpertExplanationDesiredResolution")
                        .HasColumnType("longtext");

                    b.Property<string>("ExpertExplanationEvidence")
                        .HasColumnType("longtext");

                    b.Property<string>("ExpertExplanationReason")
                        .HasColumnType("longtext");

                    b.Property<Guid?>("HandlerStaffId")
                        .HasColumnType("char(36)");

                    b.Property<string>("HistoryLogsJson")
                        .HasColumnType("longtext");

                    b.Property<string>("PartnerEvidenceUrl")
                        .HasColumnType("longtext");

                    b.Property<string>("PartnerExplanation")
                        .HasColumnType("longtext");

                    b.Property<string>("PartnerRejectionReason")
                        .HasColumnType("longtext");

                    b.Property<decimal>("PlatformFee")
                        .HasColumnType("decimal(18,2)");

                    b.Property<Guid>("ProjectId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("ReplyDeadline")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("ReportType")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<Guid>("ReporterId")
                        .HasColumnType("char(36)");

                    b.Property<string>("ReporterRole")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime(6)");

                    b.HasKey("Id");

                    b.HasIndex("HandlerStaffId");

                    b.HasIndex("ProjectId");

                    b.HasIndex("ReporterId");

                    b.ToTable("Reports");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.InteractionModule.Contract", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("ClientId")
                        .HasColumnType("char(36)");

                    b.Property<string>("ContractTerms")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("ExpertId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("ProjectId")
                        .HasColumnType("char(36)");

                    b.Property<DateTime?>("SignedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<decimal>("TotalValue")
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("Id");

                    b.HasIndex("ProjectId");

                    b.ToTable("Contracts");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.InteractionModule.Review", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("Comment")
                        .HasColumnType("longtext");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("CreatedById")
                        .HasColumnType("char(36)");

                    b.Property<string>("ExpertReply")
                        .HasColumnType("longtext");

                    b.Property<Guid>("ProjectId")
                        .HasColumnType("char(36)");

                    b.Property<int>("Rating")
                        .HasColumnType("int");

                    b.Property<DateTime?>("ReplyCreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("TargetUserId")
                        .HasColumnType("char(36)");

                    b.HasKey("Id");

                    b.HasIndex("CreatedById");

                    b.HasIndex("ProjectId");

                    b.HasIndex("TargetUserId");

                    b.ToTable("Reviews");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.InteractionModule.TransactionLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("BankAccountName")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("BankAccountNumber")
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("BankCode")
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("BankReferenceNo")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<Guid?>("DestinationWalletId")
                        .HasColumnType("char(36)");

                    b.Property<decimal>("GatewayFee")
                        .HasColumnType("decimal(18,2)");

                    b.Property<bool>("IsSandbox")
                        .HasColumnType("tinyint(1)");

                    b.Property<decimal>("PlatformFee")
                        .HasColumnType("decimal(18,2)");

                    b.Property<Guid?>("ProjectId")
                        .HasColumnType("char(36)");

                    b.Property<Guid?>("ReportId")
                        .HasColumnType("char(36)");

                    b.Property<Guid?>("SourceWalletId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime(6)");

                    b.HasKey("Id");

                    b.HasIndex("DestinationWalletId");

                    b.HasIndex("ProjectId");

                    b.HasIndex("SourceWalletId");

                    b.ToTable("TransactionLogs");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPost", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AttachmentUrl")
                        .HasColumnType("longtext");

                    b.Property<decimal>("Budget")
                        .HasColumnType("decimal(18,2)");

                    b.Property<Guid>("ClientId")
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("Deadline")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<Guid?>("DomainId")
                        .HasColumnType("char(36)");

                    b.Property<string>("DurationUnit")
                        .HasColumnType("longtext");

                    b.Property<int>("DurationValue")
                        .HasColumnType("int");

                    b.Property<string>("Implementation")
                        .HasColumnType("longtext");

                    b.Property<Guid?>("SpecializationId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("ClientId");

                    b.HasIndex("DomainId");

                    b.HasIndex("SpecializationId");

                    b.ToTable("JobPosts");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPostMiniTask", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<int>("Duration")
                        .HasColumnType("int");

                    b.Property<Guid>("JobPostTaskId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("JobPostTaskId");

                    b.ToTable("JobPostMiniTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPostSkill", b =>
                {
                    b.Property<Guid>("JobPostsId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("SkillsId")
                        .HasColumnType("char(36)");

                    b.HasKey("JobPostsId", "SkillsId");

                    b.HasIndex("SkillsId");

                    b.ToTable("JobPostSkill");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPostTask", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("JobPostId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("JobPostId");

                    b.ToTable("JobPostTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.Proposal", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("AttachmentUrl")
                        .HasColumnType("longtext");

                    b.Property<decimal>("BidAmount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("EstimatedDuration")
                        .HasColumnType("int");

                    b.Property<Guid>("ExpertId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Introduction")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<Guid>("JobPostId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Portfolio")
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("ExpertId");

                    b.HasIndex("JobPostId", "ExpertId")
                        .IsUnique();

                    b.ToTable("Proposals");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.ProposalMiniTask", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<int>("Duration")
                        .HasColumnType("int");

                    b.Property<Guid>("ProposalTaskId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("ProposalTaskId");

                    b.ToTable("ProposalMiniTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.ProposalTask", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("ProposalId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("ProposalId");

                    b.ToTable("ProposalTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.MiniTask", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("Deadline")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("FeedbackContent")
                        .HasColumnType("longtext");

                    b.Property<Guid?>("FeedbackSenderId")
                        .HasColumnType("char(36)");

                    b.Property<bool>("IsCompleted")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("ProductFile")
                        .HasColumnType("longtext");

                    b.Property<string>("ProductLink")
                        .HasColumnType("longtext");

                    b.Property<Guid>("TaskId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("FeedbackSenderId");

                    b.HasIndex("TaskId");

                    b.ToTable("MiniTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.Project", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<Guid>("ClientId")
                        .HasColumnType("char(36)");

                    b.Property<Guid?>("ConversationId")
                        .HasColumnType("char(36)");

                    b.Property<string>("DeclineReason")
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("EndDate")
                        .HasColumnType("datetime(6)");

                    b.Property<decimal>("EscrowBalance")
                        .HasColumnType("decimal(18,2)");

                    b.Property<Guid>("ExpertId")
                        .HasColumnType("char(36)");

                    b.Property<Guid?>("JobPostId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Metadata")
                        .HasColumnType("longtext");

                    b.Property<string>("ProjectFile")
                        .HasColumnType("longtext");

                    b.Property<string>("ProjectLink")
                        .HasColumnType("longtext");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("ClientId");

                    b.HasIndex("ConversationId");

                    b.HasIndex("ExpertId");

                    b.HasIndex("JobPostId");

                    b.ToTable("Projects");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.ProjectActivityLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("Action")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("ActorName")
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("varchar(1000)");

                    b.Property<Guid>("ProjectId")
                        .HasColumnType("char(36)");

                    b.HasKey("Id");

                    b.HasIndex("ProjectId");

                    b.ToTable("ProjectActivityLogs");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.ProjectExtension", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("ClientNote")
                        .HasColumnType("longtext");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<Guid>("ProjectId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)");

                    b.Property<int>("RequestedDays")
                        .HasColumnType("int");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)");

                    b.Property<Guid?>("TaskId")
                        .HasColumnType("char(36)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime(6)");

                    b.HasKey("Id");

                    b.HasIndex("ProjectId");

                    b.HasIndex("TaskId");

                    b.ToTable("ProjectExtensions");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.ProjectSkill", b =>
                {
                    b.Property<Guid>("ProjectsId")
                        .HasColumnType("char(36)");

                    b.Property<Guid>("SkillsId")
                        .HasColumnType("char(36)");

                    b.HasKey("ProjectsId", "SkillsId");

                    b.HasIndex("SkillsId");

                    b.ToTable("ProjectSkill");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.Task", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("FeedbackContent")
                        .HasColumnType("longtext");

                    b.Property<Guid?>("FeedbackSenderId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Notes")
                        .HasColumnType("longtext");

                    b.Property<Guid>("ProjectId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime(6)");

                    b.HasKey("Id");

                    b.HasIndex("FeedbackSenderId");

                    b.HasIndex("ProjectId");

                    b.ToTable("Tasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.TaskProgressLog", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("varchar(1000)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<double>("HoursWorked")
                        .HasColumnType("double");

                    b.Property<Guid>("TaskId")
                        .HasColumnType("char(36)");

                    b.HasKey("Id");

                    b.HasIndex("TaskId");

                    b.ToTable("TaskProgressLogs");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.UserModule.ApplicationUser", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<DateTime?>("AppointedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("AvatarUrl")
                        .HasColumnType("longtext");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("EmailVerificationExpiry")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("EmailVerificationToken")
                        .HasColumnType("longtext");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("PasswordResetExpiry")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("PasswordResetToken")
                        .HasColumnType("longtext");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("longtext");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("StaffCode")
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.UserModule.ExpertProfile", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("char(36)");

                    b.Property<string>("Bio")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Category")
                        .HasColumnType("longtext");

                    b.Property<string>("Certifications")
                        .HasColumnType("longtext");

                    b.Property<decimal>("HourlyRate")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Industry")
                        .HasColumnType("longtext");

                    b.Property<string>("JobTitle")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Location")
                        .HasColumnType("longtext");

                    b.Property<string>("Major")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Phone")
                        .HasColumnType("longtext");

                    b.Property<string>("PortfolioUrls")
                        .HasColumnType("longtext");

                    b.Property<decimal>("ReputationCredit")
                        .HasColumnType("decimal(18,2)");

                    b.Property<double>("SuccessRate")
                        .HasColumnType("double");

                    b.Property<string>("Website")
                        .HasColumnType("longtext");

                    b.HasKey("UserId");

                    b.ToTable("ExpertProfiles");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.UserModule.Wallet", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("char(36)");

                    b.Property<decimal>("Balance")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("EscrowBalance")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("TotalEarned")
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("UserId");

                    b.ToTable("Wallets");
                });

            modelBuilder.Entity("AITasker_Modular.Database.ProposalAiChat", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "Expert")
                        .WithMany()
                        .HasForeignKey("ExpertId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.JobModule.JobPost", "JobPost")
                        .WithMany()
                        .HasForeignKey("JobPostId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Expert");

                    b.Navigation("JobPost");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.DomainExpertProfile", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.CategoryTagModule.Domain", "Domain")
                        .WithMany()
                        .HasForeignKey("DomainId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.UserModule.ExpertProfile", "ExpertProfile")
                        .WithMany()
                        .HasForeignKey("ExpertProfilesUserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Domain");

                    b.Navigation("ExpertProfile");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.ExpertProfileSkill", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ExpertProfile", "ExpertProfile")
                        .WithMany("ExpertProfileSkills")
                        .HasForeignKey("ExpertProfilesUserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.CategoryTagModule.Skill", "Skill")
                        .WithMany("ExpertProfileSkills")
                        .HasForeignKey("SkillsId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("ExpertProfile");

                    b.Navigation("Skill");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.Specialization", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.CategoryTagModule.Domain", "Domain")
                        .WithMany("Specializations")
                        .HasForeignKey("DomainId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Domain");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ChatModule.Conversation", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "Client")
                        .WithMany()
                        .HasForeignKey("ClientId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "Expert")
                        .WithMany()
                        .HasForeignKey("ExpertId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.JobModule.JobPost", "OriginJobPost")
                        .WithMany()
                        .HasForeignKey("OriginJobPostId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.Navigation("Client");

                    b.Navigation("Expert");

                    b.Navigation("OriginJobPost");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ChatModule.Message", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.ChatModule.Conversation", "Conversation")
                        .WithMany()
                        .HasForeignKey("ConversationId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "Sender")
                        .WithMany()
                        .HasForeignKey("SenderId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Conversation");

                    b.Navigation("Sender");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.DisputeModule.Dispute", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "HandlerStaff")
                        .WithMany()
                        .HasForeignKey("HandlerStaffId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Project", "Project")
                        .WithMany()
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("HandlerStaff");

                    b.Navigation("Project");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.DisputeModule.Report", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "HandlerStaff")
                        .WithMany()
                        .HasForeignKey("HandlerStaffId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Project", "Project")
                        .WithMany()
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "Reporter")
                        .WithMany()
                        .HasForeignKey("ReporterId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("HandlerStaff");

                    b.Navigation("Project");

                    b.Navigation("Reporter");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.InteractionModule.Contract", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Project", "Project")
                        .WithMany()
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Project");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.InteractionModule.Review", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "CreatedBy")
                        .WithMany()
                        .HasForeignKey("CreatedById")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Project", "Project")
                        .WithMany()
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "TargetUser")
                        .WithMany()
                        .HasForeignKey("TargetUserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("CreatedBy");

                    b.Navigation("Project");

                    b.Navigation("TargetUser");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.InteractionModule.TransactionLog", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.Wallet", "DestinationWallet")
                        .WithMany()
                        .HasForeignKey("DestinationWalletId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Project", "Project")
                        .WithMany()
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("AITasker_Modular.Modules.UserModule.Wallet", "SourceWallet")
                        .WithMany()
                        .HasForeignKey("SourceWalletId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.Navigation("DestinationWallet");

                    b.Navigation("Project");

                    b.Navigation("SourceWallet");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPost", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "ClientUser")
                        .WithMany()
                        .HasForeignKey("ClientId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.CategoryTagModule.Domain", "Domain")
                        .WithMany()
                        .HasForeignKey("DomainId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("AITasker_Modular.Modules.CategoryTagModule.Specialization", "Specialization")
                        .WithMany()
                        .HasForeignKey("SpecializationId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.Navigation("ClientUser");

                    b.Navigation("Domain");

                    b.Navigation("Specialization");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPostMiniTask", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.JobModule.JobPostTask", "JobPostTask")
                        .WithMany("JobPostMiniTasks")
                        .HasForeignKey("JobPostTaskId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobPostTask");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPostSkill", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.JobModule.JobPost", "JobPost")
                        .WithMany("JobPostSkills")
                        .HasForeignKey("JobPostsId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.CategoryTagModule.Skill", "Skill")
                        .WithMany("JobPostSkills")
                        .HasForeignKey("SkillsId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("JobPost");

                    b.Navigation("Skill");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPostTask", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.JobModule.JobPost", "JobPost")
                        .WithMany("JobPostTasks")
                        .HasForeignKey("JobPostId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobPost");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.Proposal", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "Expert")
                        .WithMany()
                        .HasForeignKey("ExpertId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.JobModule.JobPost", "JobPost")
                        .WithMany()
                        .HasForeignKey("JobPostId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Expert");

                    b.Navigation("JobPost");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.ProposalMiniTask", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.JobModule.ProposalTask", "ProposalTask")
                        .WithMany("ProposalMiniTasks")
                        .HasForeignKey("ProposalTaskId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ProposalTask");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.ProposalTask", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.JobModule.Proposal", "Proposal")
                        .WithMany("ProposalTasks")
                        .HasForeignKey("ProposalId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Proposal");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.MiniTask", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "FeedbackSender")
                        .WithMany()
                        .HasForeignKey("FeedbackSenderId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Task", "Task")
                        .WithMany("MiniTasks")
                        .HasForeignKey("TaskId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("FeedbackSender");

                    b.Navigation("Task");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.Project", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "Client")
                        .WithMany()
                        .HasForeignKey("ClientId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.ChatModule.Conversation", "Conversation")
                        .WithMany()
                        .HasForeignKey("ConversationId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "Expert")
                        .WithMany()
                        .HasForeignKey("ExpertId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.JobModule.JobPost", "JobPost")
                        .WithMany()
                        .HasForeignKey("JobPostId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.Navigation("Client");

                    b.Navigation("Conversation");

                    b.Navigation("Expert");

                    b.Navigation("JobPost");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.ProjectActivityLog", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Project", "Project")
                        .WithMany()
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Project");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.ProjectExtension", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Project", "Project")
                        .WithMany()
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Task", "Task")
                        .WithMany()
                        .HasForeignKey("TaskId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.Navigation("Project");

                    b.Navigation("Task");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.ProjectSkill", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Project", "Project")
                        .WithMany("ProjectSkills")
                        .HasForeignKey("ProjectsId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AITasker_Modular.Modules.CategoryTagModule.Skill", "Skill")
                        .WithMany("ProjectSkills")
                        .HasForeignKey("SkillsId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Project");

                    b.Navigation("Skill");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.Task", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", "FeedbackSender")
                        .WithMany()
                        .HasForeignKey("FeedbackSenderId")
                        .OnDelete(DeleteBehavior.NoAction);

                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Project", "Project")
                        .WithMany("Tasks")
                        .HasForeignKey("ProjectId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("FeedbackSender");

                    b.Navigation("Project");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.TaskProgressLog", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.ProjectModule.Task", "Task")
                        .WithMany()
                        .HasForeignKey("TaskId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Task");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.UserModule.ExpertProfile", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", null)
                        .WithOne()
                        .HasForeignKey("AITasker_Modular.Modules.UserModule.ExpertProfile", "UserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();
                });

            modelBuilder.Entity("AITasker_Modular.Modules.UserModule.Wallet", b =>
                {
                    b.HasOne("AITasker_Modular.Modules.UserModule.ApplicationUser", null)
                        .WithOne()
                        .HasForeignKey("AITasker_Modular.Modules.UserModule.Wallet", "UserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.Domain", b =>
                {
                    b.Navigation("Specializations");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.CategoryTagModule.Skill", b =>
                {
                    b.Navigation("ExpertProfileSkills");

                    b.Navigation("JobPostSkills");

                    b.Navigation("ProjectSkills");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPost", b =>
                {
                    b.Navigation("JobPostSkills");

                    b.Navigation("JobPostTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.JobPostTask", b =>
                {
                    b.Navigation("JobPostMiniTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.Proposal", b =>
                {
                    b.Navigation("ProposalTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.JobModule.ProposalTask", b =>
                {
                    b.Navigation("ProposalMiniTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.Project", b =>
                {
                    b.Navigation("ProjectSkills");

                    b.Navigation("Tasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.ProjectModule.Task", b =>
                {
                    b.Navigation("MiniTasks");
                });

            modelBuilder.Entity("AITasker_Modular.Modules.UserModule.ExpertProfile", b =>
                {
                    b.Navigation("ExpertProfileSkills");
                });
#pragma warning restore 612, 618
        }
    }
}
