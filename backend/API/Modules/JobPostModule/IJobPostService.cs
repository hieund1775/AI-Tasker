...
namespace AITasker_Modular.Modules.JobPostModule
{
    ...
    {
        Task<PagedResult<JobPost>> GetFilteredJobsAsync(string? search, decimal? minBudget, decimal? maxBudget, string? status, Guid? categoryDomainId, int page, int pageSize);
        Task<PagedResult<JobPost>> GetJobsAsync(int page, int pageSize);
