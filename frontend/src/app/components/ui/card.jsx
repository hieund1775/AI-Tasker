...
function Card({ className, ...props }) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col rounded-xl border border-border/70 shadow-sm transition-all duration-300 hover:shadow-md hover:border-accent/20 hover:-translate-y-0.5",
        className,
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }) {
  return (
    <div
      ...
      className={cn("flex items-center gap-3 px-5 pb-5 [.border-t]:pt-5", className)}
      {...props}
    />
  );
}
