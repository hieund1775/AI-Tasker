...
function generateClientUseCases(userMessage, fileNames) {
  ...
  let useCases = [
    {
      title: "Thiết lập cơ sở dữ liệu & Xác thực người dùng",
      description:
        "Cài đặt JWT Auth, lưu trữ phiên trò chuyện và thiết kế cơ sở dữ liệu lịch sử chat.",
      originalDurationDays: 5,
    },
    {
      title: "Phân tách & Nhúng tài liệu (Document Embedding)",
      description:
        "Xây dựng cổng upload tài liệu PDF/TXT, triển khai thuật toán phân tách câu và lưu vector embeddings.",
      originalDurationDays: 7,
    },
    {
      title: "Cấu hình LLM & Prompt RAG API",
      description:
        "Tích hợp thư viện LangChain để gọi LLM và triển khai hệ thống kiểm duyệt câu trả lời (guardrails).",
      originalDurationDays: 6,
    },
    {
      title: "Giao diện Chatbot Real-time & Stream Response",
      description:
        "Thiết kế giao diện chat responsive, hiệu ứng typing indicator, stream text từ API và định dạng markdown.",
      originalDurationDays: 6,
    },
  ];
  ...
  if (
    ...
    useCases = [
      {
        title: "Đọc & Chuẩn hóa ảnh DICOM",
        description:
          "Phát triển bộ đọc tệp tin y tế DICOM, trích xuất metadata và lưu trữ thông tin ảnh vào cơ sở dữ liệu.",
        originalDurationDays: 5,
      },
      {
        title: "Huấn luyện & Đánh giá Model PyTorch",
        description:
          "Huấn luyện mô hình phân loại tổn thương phổi trên PyTorch, tối ưu hóa độ nhạy đạt tối thiểu 94%.",
        originalDurationDays: 12,
      },
      {
        title: "Giao diện vẽ Box & Đánh dấu cho bác sĩ",
        description:
          "Thiết kế canvas vẽ bounding box và vùng phân đoạn ảnh trực quan trên giao diện web.",
        originalDurationDays: 8,
      },
      {
        title: "Tự động xuất báo cáo chẩn đoán PDF",
        description:
          "Hệ thống tự động tổng hợp kết quả của AI và tạo tệp tin PDF báo cáo y tế chuẩn hóa.",
        originalDurationDays: 5,
      },
    ];
  } else if (
    ...
    useCases = [
      {
        title: "Pipeline thu thập giao dịch thời gian thực",
        description:
          "Thiết lập consumer Kafka xử lý dữ liệu giao dịch đầu vào với tốc độ 50K events/giây.",
        originalDurationDays: 8,
      },
      {
        title: "Huấn luyện Model phát hiện gian lận",
        description:
          "Xây dựng và tối ưu hóa mô hình phân lớp XGBoost/LightGBM dựa trên các đặc trưng hành vi giao dịch.",
        originalDurationDays: 10,
      },
      {
        title: "Dashboard giải thích quyết định của AI (XAI)",
        description:
          "Hiển thị giá trị đóng góp SHAP/LIME để giải thích lý do giao dịch bị đánh dấu gian lận cho nhân viên kiểm duyệt.",
        originalDurationDays: 8,
      },
      {
        title: "Hệ thống cảnh báo & Tự động khóa thẻ",
        description:
          "Kích hoạt cảnh báo thời gian thực qua SMS/Email và tự động gửi lệnh khóa tài khoản/thẻ có dấu hiệu rủi ro cao.",
    ...
    useCases = [
      {
        title: "Thu thập hành vi tương tác của người dùng",
        description:
          "Ghi nhận lượt xem sản phẩm, đánh giá và lịch sử mua sắm theo thời gian thực.",
        originalDurationDays: 6,
      },
      {
        title: "Huấn luyện mô hình Collaborative Filtering",
        description:
          "Xây dựng mô hình Matrix Factorization hoặc Two-Tower Neural Network để dự đoán sản phẩm gợi ý.",
        originalDurationDays: 8,
      },
      {
        title: "Triển khai hạ tầng A/B Testing",
        description:
          "Phân luồng người dùng thử nghiệm và đo lường sự thay đổi của tỷ lệ chuyển đổi đơn hàng.",
        originalDurationDays: 6,
      },
      {
        title: "API gợi ý sản phẩm độ trễ thấp (<100ms)",
        description:
          "Xây dựng hệ thống cache Redis để phục vụ danh sách gợi ý sản phẩm ngay khi tải trang.",
        originalDurationDays: 5,
      },
    ];
  } else if (
    ...
    useCases = [
      {
        title: "RL Environment & Wrapper",
        description:
          "Expose NPC states, actions, rewards into a Python training script wrapper.",
        originalDurationDays: 10,
      },
      {
        title: "Multi-Agent Policy Training",
        description:
          "Train agents using PPO/SAC with attention models for social behaviors.",
        originalDurationDays: 14,
      },
      {
        title: "UE5 PyTorch C++ Plugin",
        description:
          "Create plugin wrapping PyTorch lib to load models and query actions at 60 FPS.",
        originalDurationDays: 10,
      },
    ];
  } else if (
    ...
    useCases = [
      {
        title: "Phân tích yêu cầu & Thiết kế kiến trúc",
        description:
          "Vẽ sơ đồ luồng nghiệp vụ, đặc tả cơ sở dữ liệu ERD và định nghĩa các tài liệu thiết kế.",
        originalDurationDays: 4,
      },
      {
        title: "Triển khai Cơ sở dữ liệu & Viết Backend API",
        description:
          "Viết migrations, cấu hình database và hoàn thiện các API Endpoint CRUD cơ bản.",
        originalDurationDays: 8,
      },
      {
        title: "Tích hợp giao diện Frontend",
        description:
          "Tạo các trang màn hình, kết nối form dữ liệu với API và quản lý state tập trung.",
        originalDurationDays: 8,
      },
      {
        title: "Kiểm thử hệ thống & Bàn giao",
        description:
          "Chạy các bài kiểm thử tích hợp (Integration Tests) và deploy dự án lên môi trường staging để nghiệm thu.",
        originalDurationDays: 5,
      },
    ];
  }
...
export function AIClientsUseCasePlanner({
  ...
  const handleSend = useCallback(() => {
    ...
    const userMsgText =
      trimmed ||
      `Hãy phân tích tài liệu đính kèm để sinh Use Cases: ${files.map((f) => f.name).join(", ")}`;
  ...
  const handleFilesChange = useCallback((newFiles) => {
    ...
    if (newFiles.length > 0) {
      ...
      setMessages((prev) => [
        ...
        {
          role: "user",
          text: `Đã đính kèm tài liệu: ${names}`,
          timestamp: Date.now(),
        },
      ]);
    }
          ...
          {messages.length === 0 && !loading && (
            <div className="text-center py-8 px-4">
              ...
              <p className="text-sm text-muted-foreground font-medium">
                Hãy nhắn tin hoặc tải lên tài liệu để AI tự động lập luồng Use
                Cases dự án.
              </p>
              <p className="text-xs text-muted-foreground/60 mt-1 italic">
                Ví dụ: "Tôi muốn làm chatbot hỗ trợ khách hàng tích hợp RAG"
              </p>
            </div>
          )}
                  ...
                  <div className="mt-4 space-y-3 border-t border-border pt-3">
                    ...
                    <div className="space-y-2">
                      ...
                      {msg.plan.useCases.map((uc, index) => (
                        <div
                          key={index}
                          className="bg-secondary/60 border border-border rounded-lg p-2.5 text-xs space-y-1"
                        >
              ...
              <div className="bg-card border border-border rounded-xl rounded-bl-md px-4 py-3 shadow-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground font-medium">
                    AI đang phân tích tài liệu & yêu cầu...
                  </span>
                </div>
              </div>
            </div>
      ...
      {generatedPlan && !loading && (
        <div className="shrink-0 bg-card border-t border-border p-3 flex flex-col gap-2">
          ...
          >
            ...
            {applied
              ? "✓ Đã áp dụng Use Cases vào Form"
              : "Áp dụng Use Cases này"}
          </button>
        </div>
      )}
