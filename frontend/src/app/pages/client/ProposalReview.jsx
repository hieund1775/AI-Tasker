...
export function ProposalReview() {
  ...
  const handleAccept = (proposalId, expertName) => {
    ...
    if (result.success) {
      ...
      setFeedback({
        type: "error",
        message: result.error || "Failed to accept.",
      });
    }
  ...
  const handleDecline = (proposalId, expertName) => {
    ...
    if (result.success) {
      ...
      setFeedback({
        type: "error",
        message: result.error || "Failed to decline.",
      });
    }
    setTimeout(() => setFeedback(null), 5000);
  };
        ...
        <div className="p-8 border-b border-gray-100">
          <div className="flex items-start justify-between flex-wrap gap-4">
            ...
            <span className="flex-shrink-0 px-4 py-2 bg-purple-50 text-purple-700 rounded-xl text-sm font-semibold inline-flex items-center gap-2">
              ...
              {visibleProposals.length} proposal
              {visibleProposals.length !== 1 ? "s" : ""} received
            </span>
          </div>
        </div>
      ...
      {visibleProposals.length === 0 ? (
        ...
        <div className="space-y-4">
          {visibleProposals.map((proposal) => {
            ...
            const hasBeenActed =
              isAccepted || isDeclined || actedIds.has(proposal.id);
