...
export async function handleMockRequest(endpoint, method, body, authenticated, token) {
  ...
  try {
    ...
    if (method === "PUT") {
      ...
      if (submitId) {
        const updated = updateProject(submitId, {
          status: "completed",
          projectLink: body?.projectLink || query.projectLink,
        });
        return { success: true, projectId: submitId, status: "completed" };
      }
    }
    ...
    if (reportIdMatch && method === "PUT") {
      ...
      if (newStatus === "Accepted" || newStatus === "accept") {
        updateReport(report.id, { status: "Accepted", adminNote: body.adminNote || body.note || "", resolvedAt: new Date().toISOString() });
        return { success: true, reportId: report.id, status: "Accepted" };
      }
      if (newStatus === "Rejected" || newStatus === "reject") {
        updateReport(report.id, { status: "Rejected", adminNote: body.reason || body.note || "", resolvedAt: new Date().toISOString() });
